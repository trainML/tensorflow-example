...
def run(flags_obj):
  ...
  """
    keras_utils.set_session_config(
        enable_eager=flags_obj.enable_eager, enable_xla=flags_obj.enable_xla
    )
    ...
    if dtype == "fp16":
        raise ValueError(
            "dtype fp16 is not supported in Keras. Use the default " "value(fp32)."
        )
    ...
    num_eval_steps = (
        cifar_preprocessing.NUM_IMAGES["validation"] // flags_obj.batch_size
    )
